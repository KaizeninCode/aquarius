...
const DeliveryDetail = () => {
  ...
  const handleNavigate =  () => {
    ...
    Linking.openURL(url).catch(error => {
      ...
      Alert.alert('Could not open maps. Please check that Google Maps is installed on your device.')
    })
  }

  const handleCall = () => {
    if(!job?.customerPhone) return
    Linking.openURL(`tel:${job.customerPhone}`).catch(error => console.error('Failed to open dialer: ', error))
  }
  ...
  return (
    <SafeAreaView className='flex-1 bg-slate-50 p-4'>
      ...
      <View className="border border-slate-200 rounded-xl p-4 mb-4">
        ...
        <Text className="text-base font-semibold">{job.deliveryAddress.label}</Text>
        {job.deliveryAddress.notes ? (
          <Text className="text-slate-500 text-sm mt-1">{job.deliveryAddress.notes}</Text>
        ) : null}
      ...
      <View className="border border-slate-200 rounded-xl p-4 mb-6">
        ...
        {job.items.map((item, i) => (
          <View key={i} className="flex-row justify-between mb-1">
            ...
            <Text className="text-slate-700">KES {item.price * item.quantity}</Text>
          </View>
        ))}
